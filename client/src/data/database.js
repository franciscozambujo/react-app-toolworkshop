...
export async function getCarChecks() {
  const [rows] = await pool.query(
    "SELECT r.id,(SELECT u.nome FROM utilizadores u WHERE u.id = r.cliente) AS cliente,r.carro,  r.matricula,  r.data_agendada,  r.estado FROM revisoes r;"
  );
  return rows;
}
...
export async function getLastReview() {
  const [rows] = await pool.query(
    "SELECT id, nome, email, descricao, estrelas FROM avaliacoes ORDER BY id DESC LIMIT 1;"
  );
  return rows;
}
...
export async function getPenultimateReview() {
  const [rows] = await pool.query(
    "SELECT id, nome, email, descricao, estrelas FROM avaliacoes WHERE id = (SELECT id FROM avaliacoes ORDER BY id DESC LIMIT 1 OFFSET 1);"
  );
  return rows;
}
...
export async function getAntepenultimateReview() {
  const [rows] = await pool.query(
    "SELECT id, nome, email, descricao, estrelas FROM avaliacoes WHERE id = (SELECT id FROM avaliacoes ORDER BY id DESC LIMIT 1 OFFSET 2);"
  );
  return rows;
}
...
export async function getAntantepenultimateReview() {
  const [rows] = await pool.query(
    "SELECT id, nome, email, descricao, estrelas FROM avaliacoes WHERE id = (SELECT id FROM avaliacoes ORDER BY id DESC LIMIT 1 OFFSET 3);"
  );
  return rows;
}
...
export async function getUsersRole(user) {
  const query =
    "SELECT utilizadores.cargo FROM utilizadores WHERE utilizadores.user = ?;";
  const [rows] = await pool.query(query, [user]);
...
export async function getUsersPass(user) {
  const query =
    "SELECT utilizadores.password, utilizadores.cargo FROM utilizadores WHERE utilizadores.user = ?;";
  const [rows] = await pool.query(query, [user]);
  return rows;
}
...
export async function createCarClientByEmail(
  carBrand,
  carModel,
  carPlate,
  clientEmail
